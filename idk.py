...
class Simulation:
    ...
    def run(self):
        for chunk in self.data_handler.stream_data():
            try:
                prices = chunk['price'].values.astype(np.float32)
                volumes = chunk['volume'].values.astype(np.float32)
                ...
                if self.use_gpu:
                    score = self.predictor.predict(cp.asarray(prices), cp.asarray(volumes))
                else:
                    score = compute_zscore_cpu(prices, volumes)

                action = self.decision_engine.decide(score)
                self.execution_engine.execute(action, prices[-1], timestamps[-1])
